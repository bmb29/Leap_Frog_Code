...
condition3(u,t,integrator)=abs(u[1])>barrier || abs(u[2])>barrier || abs(u[3])>barrier||abs(u[4])>barrier
affect3!(integrator) = terminate!(integrator)
# Restrict to energy surface
cb1 = ManifoldProjection(g,save=false)
# only keep plane X=0
cb2 = ContinuousCallback(condition2,affect!,nothing)
# stop when vortices escape to infinity
...
ArrQ=range(BigFloat("1e-5"),stop=BigFloat("1"),length=n_iter)
ArrP=range(BigFloat("1e-5"),stop=BigFloat("1"),length=n_iter)
#initialize stuff
                    ...
                    if ~isempty(Y) && Y>0
                        ...
                        if dH_max<1e-8# t,A=solve(prob, RK4(),maxiters=1e20, reltol=1e-6,abstol=1e-8,callback=cb,save_start=true,save_end=true,save_everystep=false)
                            ...
                            #testing if energy is conserved at end?
                                ...
                                if abs(A[1,end])<1e-7 && how_far>10
                                    push!(Last_Q,A[3,end])
                                    push!(Last_P,A[2,end])
                                    push!(Last_X,A[1,end])
                                    push!(Last_Y,A[4,end])
                                    push!(Last_T,sol.t[end])
                                elseif abs(A[1,end-1])<1e-7 && how_far>10
...
H_vec=zeros(BigFloat,length(Last_P))
u_max=zeros(BigFloat,length(Last_P))
