...
function escape_exit_num_dimer(mesh_list, t_end, H)
    ...
    if ~isempty(P1)
       ...
       #constructor for ODE
        ...
        # sol=solve(prob, Vern9(),maxiters=1e20, reltol=1e-10,abstol=1e-12,callback=cb,save_start=false,save_end=true,save_everystep=false)
        ...
        if maximum([abs(uf[1]),abs(uf[2]),abs(uf[4]),abs(uf[5])]) > barrier
            return uf[3]
        else
            return -1
