...
@everywhere begin
    ...
    n_iter_Q = 501;n_iter_P =Int(round(4.5/5*n_iter_Q));
    N = n_iter_P * n_iter_Q;
...
while count <= length(H)
    ...
    @everywhere h_BSON="FORWARD_Hit_for_escape_data"*h*nQ*nP*".bson"
    @everywhere h_BSON_small="FORWARD_small_Hit_for_escape_data"*h*nQ*nP*".bson"


    @everywhere file_name = "HIT_Escape_" * h * ".fig"
    println(file_name)
    num_until_exit = @showprogress pmap(escape_dimerB.escape_exit_num_dimer, mesh_list, t_end, Energy)
